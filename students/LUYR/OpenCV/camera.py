...
def overlay_on_image(display_image, filtered_objects):
    global num_person
    num_person = len(filtered_objects)
    ...
    while (True):
        while (True):
            ...
            if ((video_device == None) or (not video_device.isOpened())):
                ...
                print ('file name:' + input_video_file_name)
            ...
            while True :
                ...
                for i in range(3):
                    for j in range(3):
                        cv2.rectangle(input_image,(i*100+X0,j*100+Y0),(i*100+100+X0,j*100+100+Y0),(255,128,255),5)

                with open('test1.txt', 'w') as f1:
                    f1.write(str(num_person))

                #cv2.putText(input_image,'there are '+str(num_person)+' person',(0,40),cv2.FONT_HERSHEY_SIMPLEX,1.2,(255,255,255),2)
                if (not ret_val):
                    end_time = time.time()
                ...
                print("there are " + str(num_person) +" person")
